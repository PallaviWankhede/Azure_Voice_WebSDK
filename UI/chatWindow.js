...
// hoonartek customization starts for country selection template for enable done after any one checkbox selected ends
    ...
    return function koreBotChat() {
            console.log("In function korechatbot");
	...
	// hoonartek kore Customization for mic
        ...
        var returnFun =(function ($,KRPerfectScrollbar) {
            ...
            var _ttsContext = null, _ttsConnection = null, ttsServerUrl = '', ttsAudioSource = null, _txtToSpeak = "", optionIndex = 65; audioMsgs = []; speechSyn = null; window.audioPlaying = false;   // Audio context
	...
	//hoonartek kore customization for mic on off
            function readDigitsSeparately(numberString) {
                console.log("In readDigitsSeparately");
	...
	function reFormatUserText(text) {
		...
		// hoonartek customization for health discount template & country selection template done-button
		 ...
		 // kore customization starts for the travel country selection template 
                ...
                _chatContainer.off('click', '.close-btn').on('click', '.close-btn', function (event) {
                    console.log("In off click close btn");
                    $('.recordingMicrophone').trigger('click');
                    if (ttsAudioSource) {
                        ttsAudioSource.stop();
                    }
		...
		// hoonartek customization for clear history starts after cross button close button
                ...
                _chatContainer.off('click', '.minimize-btn').on('click', '.minimize-btn', function (event) {
                    console.log("In off click mini btn");
                    if(me.config.multiPageApp && me.config.multiPageApp.enable){
                ...
                _chatContainer.off('click', '.expand-btn').on('click', '.expand-btn', function (event) {
                    console.log("In off click expand btn");
 ...
 //            }
		...
		// pallavi working
            ...
            //     console.log("In prototype speakWithAzure");
            //     if (!_txtToSpeak) {
            //         return false;
            //     }
                
            //     if (typeof window.speakTextWithAzure === 'function') {
            //         console.log("In window.speakTextwithAzure calling");
            //         console.log("_txtToSpeak - " + _txtToSpeak);

            //         // window.speakTextWithAzure(_txtToSpeak); //pallu UC
            //         audioMsgs.push(_txtToSpeak); //pallu added
            //         playMessageSequence(); //pallu added
            //     } else {
            //         console.warn("KORE: Azure TTS is not properly initialized");
            //     }
            // };
            // chatWindow.prototype.stopSpeaking= function() {
            //     console.log("In stopSpeaking azure");
            //     var me = this;
            //     if (me.config.isTTSEnabled) {
            //         if(me.config.ttsInterface && me.config.ttsInterface==="webapi"){
            //             if('speechSynthesis' in window){
            ...
            //                 console.log("In stopSpeakingTTS azure");
            //                 audioMsgs = [];       //pallu
            //                 audioPlaying = false; // pallu
            ...
            //     console.log("In playMessageSequence");
            ...
            //     // Proceed only if there are messages to play
            //     if (audioMsgs.length > 0) {
            //         console.log("Audiomsgs all message:", audioMsgs);
            //         console.log("Playing message:", audioMsgs[0]);  // Log the message being played
            //         audioPlaying = true;  // Set the flag that audio is playing
            //         let messageToSpeak = audioMsgs.shift();  // Get the next message to speak
            
            //         // Decide whether to turn off microphone based on specific messages
            //         if (
            //             messageToSpeak === 'Please select the value manually' ||
            //             messageToSpeak === 'Please select the options manually' ||
            //             messageToSpeak === 'Please fill out the form manually'
            //         ) {
            //             conMicOff = true;
            //         } else {
            //             conMicOff = false;
            ...
            //         // Use Azure TTS to speak the message
            //         window.speakTextWithAzure(messageToSpeak);
            
            //         // Monitor when speech is done to continue with the next message
            //         window.speakTextWithAzure.onSpeechSynthesisComplete = function() {
            //             console.log("Speech finished for message:", messageToSpeak);
            //             audioPlaying = false;  // Mark the message as played
            //             playMessageSequence();  // Continue with the next message
            //         };
                    
            //     } else {
            //         console.log("No messages in audioMsgs to play.");
            //     }
            //     if (recognizing && audioPlaying) {
            //                 recognition.stop();
            //             } else if (sessionStorage.getItem("mic") === 'true' && !recognizing && !audioPlaying && !conMicOff) {
            //                 recognition.start();
            //             }
            // }
            ...
            // // Function to play messages sequentially
            // function playMessageSequence() {
            //     console.log("Queue:", audioMsgs);
            //     if (audioMsgs.length > 0 && !audioPlaying) {
            //         audioPlaying = true;
            //         let message = audioMsgs[0];  // Get the first message
            //         console.log("Starting speech:", message);
            //         window.speakTextWithAzure(message);
            //     }
            // }
            ...
            // window.playMessageSequence = playMessageSequence;
            // pallavi-azure used
		...
		//         if (
		//             speechSyn.text === 'Please select the value manually' ||
		//             speechSyn.text === 'Please select the options manually' ||
		//             speechSyn.text === 'Please fill out the form manually'
		//         ) {
		//             conMicOff = true;
		//         } else {
		//             conMicOff = false;
		...
		//         loadVoices(voices => {
        //             console.log("In loadvoice emma");
		//             // Try to find "Microsoft Emma"
		//             const microsoftEmma = voices.find(voice => voice.name === "Microsoft Emma Online (Natural) - English (United States)");
		
		//             // Use "Microsoft Emma" if available, otherwise use the default or first available voice
		//             speechSyn.voice = microsoftEmma || voices.find(voice => voice.default) || voices[0];
		...
		//             console.log(`Using voice: ${speechSyn.voice.name}`);
		//             console.log(`Voice language: ${speechSyn.voice.lang}`);
		//             console.log(`Voice URI: ${speechSyn.voice.voiceURI}`);
